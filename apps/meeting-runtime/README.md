...
This optional local sidecar powers Oxmux "背后听写" on desktop. It binds only
...
```bash
PYTORCH_ENABLE_MPS_FALLBACK=1 OXMUX_VALUE_DEVICE=mps \
  TOKENIZERS_PARALLELISM=false .venv/bin/python -u smoke_value_model.py
```

Use `OXMUX_VALUE_DEVICE=cpu` on systems without Apple Silicon MPS or CUDA.

The first request downloads `OpenMOSS-Team/MOSS-Transcribe-Diarize` and
`openbmb/MiniCPM5-1B`. Set `OXMUX_MOSS_MODEL`, `OXMUX_VALUE_MODEL`,
`OXMUX_MOSS_DEVICE`, or `OXMUX_VALUE_DEVICE` to use a local cache or selected
device. `OXMUX_MEETING_DEVICE` remains a compatibility override for both. The
...
your private deployment with `OXMUX_MEETING_MAX_AUDIO_BYTES`. Its default CORS
policy accepts the desktop shell and loopback development pages. A hosted web
client must explicitly set `OXMUX_MEETING_ALLOWED_ORIGINS` to its exact HTTPS
origin. Set `OXMUX_MEETING_RUNTIME_TOKEN` for every LAN deployment.
...
sends value-bearing text to Oxmux; original audio remains on the device.

The same client can instead use a private runtime endpoint when configured.
It intentionally refuses to start without a Oxmux API URL and login
...
private Wi-Fi/VPN address, enable `OXMUX_MEETING_RUNTIME_TOKEN`, and terminate
