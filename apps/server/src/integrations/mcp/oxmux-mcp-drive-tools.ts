// [INPUT]: Agent 身份（OxmuxMcpContext）+ Drive 工具调用
...
import { toToolResult, type OxmuxMcpContext } from './oxmux-mcp-context'
...
const resolveMcpScope = async (ctx: OxmuxMcpContext, input: { personal: boolean; workspaceId?: string }) => {
  if (input.personal) {
...
const assertMcpFileAccess = async (ctx: OxmuxMcpContext, fileId: string, required: 'read' | 'edit' | 'manage' = 'read') => {
...
export const registerOxmuxMcpDriveTools = (server: McpServer, ctx: OxmuxMcpContext) => {
