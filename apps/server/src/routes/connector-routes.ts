...
// [POS]: connector gateway 协议层——web 与 agent 经 Oxmux server 间接访问连接器，凭据留在 runtime 边界
...
const resolveOfficialConnectorBaseUrl = () => {
  const target = getEnv('OXMUX_OFFICIAL_CONNECTOR_URL')?.trim()
...
const resolveOfficialConnectorAdminToken = () => getEnv('OXMUX_OFFICIAL_CONNECTOR_ADMIN_TOKEN')?.trim()
  || getEnv('OXMUX_OFFICIAL_CONNECTOR_ADMIN_TOKEN')?.trim()
  || ''

const resolveOfficialConnectorRuntimeToken = () => getEnv('OXMUX_OFFICIAL_CONNECTOR_RUNTIME_TOKEN')?.trim()
...
export const registerConnectorRoutes = (app: Hono, requireAuth: MiddlewareHandler) => {
  ...
  // server 按 x-oxmux-workspace（执行 workspaceId）解析 collab workspace，过滤连接。
  ...
  app.post('/api/connector/mcp', async (c) => {
    ...
    const executionWorkspaceId = c.req.header('x-oxmux-workspace')?.trim() || undefined
    const actingUserId = c.req.header('x-oxmux-acting-user')?.trim() || undefined
