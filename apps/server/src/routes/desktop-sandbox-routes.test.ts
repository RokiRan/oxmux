...
test('desktop sandbox preview uses localtest host for local dev even when project preview env points to production', () => {
  ...
  try {
    const target = buildDesktopPreviewAccessTarget({
      requestUrl: 'http://app.oxmux.localtest.me:15173/api/tasks/task-1/desktop-sandbox/open',
      headers: new Headers({
        host: 'app.oxmux.localtest.me:15173',
      }),
    ...
    assert.equal(
      target.publicHost,
      'shopping-agent-preview--desktop-preview-1.oxmux.localtest.me:18989',
    )
    assert.equal(
      target.publicUrl,
      'http://shopping-agent-preview--desktop-preview-1.oxmux.localtest.me:18989/',
    )
    assert.equal(
      target.tunnelUrl,
      'ws://app.oxmux.localtest.me:18989/api/preview-tunnels/ws',
    )
  } finally {
  ...
  try {
    const target = buildDesktopPreviewAccessTarget({
      ...
      headers: new Headers({
        ...
        referer: 'http://app.oxmux.localtest.me:15173/workspaces?panel=desktop',
      }),
      projectName: 'Shopping Agent',
    ...
    assert.equal(
      target.publicHost,
      'shopping-agent-preview--desktop-preview-2.oxmux.localtest.me:18989',
    )
    assert.equal(
      target.publicUrl,
      'http://shopping-agent-preview--desktop-preview-2.oxmux.localtest.me:18989/',
    )
    assert.equal(
      target.tunnelUrl,
