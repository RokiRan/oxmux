...
const ELECTRON_APP_ORIGIN = 'oxmux-app://local'
...
export const auth = betterAuth({
  ...
  databaseHooks: {
    // 数据隔离修复（BUG-01）：better-auth 用户写在 user 表，oxmux admin 读 users 表。
    // 注册时同步创建 oxmux users 记录，保证邮箱注册用户对 admin 用户管理可见/可管理。
    user: {
      create: {
        after: async (user) => {
          await syncBetterAuthUserToOxmux(user).catch((error) => {
            console.error('[better-auth] sync user to oxmux users failed', error)
          })
        },
      },
    },
