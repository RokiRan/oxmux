...
const resolveGitHubAppConfig = (): GitHubAppConfig | null => {
  const appId = process.env.GITHUB_APP_ID?.trim() || getEnv('OXMUX_GITHUB_APP_ID')?.trim() || ''
  const appSlug = process.env.GITHUB_APP_SLUG?.trim() || getEnv('OXMUX_GITHUB_APP_SLUG')?.trim() || ''
  const privateKey = process.env.GITHUB_APP_PRIVATE_KEY?.trim() || getEnv('OXMUX_GITHUB_APP_PRIVATE_KEY')?.trim() || ''
  ...
  const apiBaseUrl = trimTrailingSlash(
    process.env.GITHUB_APP_API_BASE_URL?.trim()
      || getEnv('OXMUX_GITHUB_APP_API_BASE_URL')?.trim()
      || 'https://api.github.com',
  )
  const webBaseUrl = trimTrailingSlash(
    process.env.GITHUB_APP_WEB_BASE_URL?.trim()
      || getEnv('OXMUX_GITHUB_APP_WEB_BASE_URL')?.trim()
      || 'https://github.com',
  )
...
const resolveGitHubAppOAuthConfig = () => {
  const clientId = process.env.GITHUB_APP_CLIENT_ID?.trim() || getEnv('OXMUX_GITHUB_APP_CLIENT_ID')?.trim() || ''
  const clientSecret = process.env.GITHUB_APP_CLIENT_SECRET?.trim() || getEnv('OXMUX_GITHUB_APP_CLIENT_SECRET')?.trim() || ''
  ...
  return {
    clientId,
    clientSecret,
    callbackUrl: process.env.GITHUB_APP_CALLBACK_URL?.trim() || getEnv('OXMUX_GITHUB_APP_CALLBACK_URL')?.trim() || '',
...
export const resolveGitHubAppCommitIdentity = (user?: GitHubAppCommitIdentityUser) => {
  ...
  const botLogin = process.env.GITHUB_APP_BOT_LOGIN?.trim()
    || getEnv('OXMUX_GITHUB_APP_BOT_LOGIN')?.trim()
    || `${config.appSlug}[bot]`
  const botEmail = process.env.GITHUB_APP_BOT_EMAIL?.trim()
    || getEnv('OXMUX_GITHUB_APP_BOT_EMAIL')?.trim()
