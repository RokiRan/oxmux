...
import { OXMUX_AGENT_OPS_REQUIRED_INSTRUCTION } from '../lib/system-skills'
...
export const buildMainAgentRuntimeToolInstructions = (runtimeId?: RuntimeId) => {
  ...
  return [
    '当前执行端是 Pi。oxmux MCP 在 Pi 中作为 custom tools 提供，并列在 Available tools 中。',
...
export const buildMainAgentSystemPrompt = (projects: Project[], userId: string) => {
  ...
  return [
    '你是 oxmux 中当前会话绑定的 Agent，请理解用户问题并给出简洁、可执行的回复。',
    OXMUX_AGENT_OPS_REQUIRED_INSTRUCTION,
    ...
    'oxmux 平台能力只通过 oxmux MCP 提供。需要真实平台状态、对象操作、系统上下文，或用户明确要求时，再调用对应 MCP。',
    '不要把界面上的当前选中项目、任务、会话当作默认上下文；除非用户明确提到，或你已经通过 oxmux MCP 查询确认，否则不要主动引用这些对象。',
    '凡是涉及项目、任务、会话、工作区的查询和修改，都必须先调用 oxmux MCP tools 获取真实状态或执行真实操作。',
    '不要用 bash、curl、端口扫描或本地文件来寻找或绕过 oxmux MCP。应从当前运行时实际提供的工具中选择 oxmux 工具；若未提供，直接说明当前工具不可用。',
...
export const buildCustomAgentSystemPrompt = (
  ...
  const identitySection = [
    '',
    '# Identity',
    `You are ${agent.name}, a persistent Agent in Oxmux.`,
  ...
  return [
    `你当前扮演 oxmux 自定义 Agent「${agent.name}」。`,
    `Agent ID: ${agent.id}`,
    `角色: ${profile.role || '未设置'}`,
