...
const baseSource = (overrides: Partial<WorkspacePreviewSourceSummary> = {}): WorkspacePreviewSourceSummary => ({
  publicUrl: 'https://myapp-preview--host1.oxmux.xyz/',
  previewHost: 'myapp-preview--host1.oxmux.xyz',
...
test('resolveListPreviewAddress: tunnel 模式显示隧道域名', () => {
  ...
  assert.equal(addr.url, 'https://myapp-preview--host1.oxmux.xyz/')
  assert.equal(addr.host, 'myapp-preview--host1.oxmux.xyz')
...
test('resolveListPreviewAddress: gateway 模式显示公网预览域名', () => {
  const addr = resolveListPreviewAddress({
    source: baseSource({ publicUrl: 'https://myapp.oxmux.xyz/', previewHost: 'myapp.oxmux.xyz' }),
    remoteTransport: 'gateway',
  })
  assert.equal(addr.transport, 'gateway')
  assert.equal(addr.host, 'myapp.oxmux.xyz')
...
test('resolveListPreviewAddress: 不同端口有独立隧道域名,切换端口时地址跟着变', () => {
  const source3000 = baseSource({ port: 3000, publicUrl: 'https://myapp-preview--host1-3000.oxmux.xyz/' })
  const source8080 = baseSource({ port: 8080, publicUrl: 'https://myapp-preview--host1-8080.oxmux.xyz/', primary: false })
  ...
  assert.equal(addr3000.host, 'myapp-preview--host1-3000.oxmux.xyz')
  assert.equal(addr8080.host, 'myapp-preview--host1-8080.oxmux.xyz')
})
