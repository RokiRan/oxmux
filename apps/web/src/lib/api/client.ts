...
export const authFetch = async (input: string, init?: RequestInit): Promise<Response> => {
  ...
  if (response.status === 401 && typeof window !== 'undefined') {
    ...
    if (path !== '/login' && sentToken) {
      ...
      // 对 dev login 用户也会返回 401，但 oxmux token 仍然有效。此时不应把整个会话登出——
      // 先校验 oxmux token 是否仍有效，只有 token 真的失效才强制登出。
      const storedToken = localStorage.getItem('auth_token')
      const tokenStillValid = storedToken
        ? await oxmuxTokenStillValid(storedToken)
        : false
      if (!tokenStillValid) {
...
/** 校验 oxmux token 是否仍有效（/api/auth/me）。401 可能是功能级（better-auth 会话缺失），token 有效则不应登出。 */
export const oxmuxTokenStillValid = async (token: string): Promise<boolean> => {
  try {
