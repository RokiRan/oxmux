...
const resolveOpenSandboxDomain = () => readEnv('OXMUX_OPENSANDBOX_DOMAIN') || readEnv('OPEN_SANDBOX_DOMAIN') || DEFAULT_OPENSANDBOX_DOMAIN
const resolveOpenSandboxProtocol = () => readEnv('OXMUX_OPENSANDBOX_PROTOCOL') === 'https' ? 'https' : 'http'
const resolveDesktopImage = () => readEnv('OXMUX_OPENSANDBOX_DESKTOP_IMAGE') || readEnv('SANDBOX_IMAGE') || DEFAULT_DESKTOP_IMAGE
const resolveCliImage = () => readEnv('OXMUX_OPENSANDBOX_CLI_IMAGE') || readEnv('CLI_SANDBOX_IMAGE') || resolveDesktopImage()
const resolveVncPassword = () => readEnv('OXMUX_OPENSANDBOX_VNC_PASSWORD') || readEnv('VNC_PASSWORD') || DEFAULT_VNC_PASSWORD
const resolveConfiguredDesktopProfile = (): WorkspaceDesktopSandboxDisplayProfile => {
  const value = readEnv('OXMUX_OPENSANDBOX_DESKTOP_PROFILE')
...
const resolveDisplaySettings = (params: {
  ...
  return {
    ...settings,
    noVncQuality: readEnvNumber('OXMUX_OPENSANDBOX_NOVNC_QUALITY', settings.noVncQuality),
    noVncCompression: readEnvNumber('OXMUX_OPENSANDBOX_NOVNC_COMPRESSION', settings.noVncCompression),
  }
}
...
const createConnectionConfig = () => new ConnectionConfig({
  ...
  apiKey: readEnv('OXMUX_OPENSANDBOX_API_KEY') || readEnv('OPEN_SANDBOX_API_KEY'),
  requestTimeoutSeconds: readEnvNumber('OXMUX_OPENSANDBOX_REQUEST_TIMEOUT_SECONDS', DEFAULT_REQUEST_TIMEOUT_SECONDS),
  debug: readEnvBoolean('OXMUX_OPENSANDBOX_DEBUG'),
  useServerProxy: readEnvBoolean('OXMUX_OPENSANDBOX_USE_SERVER_PROXY'),
})
...
const shouldAutostartOpenSandboxServer = () => {
  if (!readEnvBoolean('OXMUX_OPENSANDBOX_AUTOSTART', true)) {
    return false
  }
...
const resolveAllowedHostPaths = () => {
  const custom = readEnv('OXMUX_OPENSANDBOX_ALLOWED_HOST_PATHS')?.trim()
  if (custom) {
...
const createOpenSandboxServerConfig = (port: number) => [
  '# Generated by oxmux worker. Safe to delete; it will be recreated.',
  '[server]',
  'host = "127.0.0.1"',
  `port = ${port}`,
    ...
    if (looksLikeHtml) {
      return {
        ...
        message: [
          ...
          '请确认 OpenSandbox server 正在运行，并把 OXMUX_OPENSANDBOX_DOMAIN 指向它的 host:port。',
...
const connectDesktopSandbox = async () => {
  ...
  const sandbox = await Sandbox.connect({
    sandboxId,
    connectionConfig: createConnectionConfig(),
    readyTimeoutSeconds: readEnvNumber('OXMUX_OPENSANDBOX_READY_TIMEOUT_SECONDS', 60),
  })
...
const createDesktopSandbox = async (request?: Pick<WorkspaceDesktopSandboxRequest, 'displayProfile' | 'clientNetwork' | 'cwd'>) => {
  ...
  try {
    ...
    createdSandbox = await Sandbox.create({
      image,
      connectionConfig: createConnectionConfig(),
      timeoutSeconds: readEnvNumber('OXMUX_OPENSANDBOX_DESKTOP_TIMEOUT_SECONDS', DEFAULT_DESKTOP_TIMEOUT_SECONDS),
...
const createCliSandbox = async () => {
  ...
  try {
    ...
    createdSandbox = await Sandbox.create({
      image,
      connectionConfig: createConnectionConfig(),
      timeoutSeconds: readEnvNumber('OXMUX_OPENSANDBOX_CLI_TIMEOUT_SECONDS', DEFAULT_CLI_TIMEOUT_SECONDS),
      metadata: {
        source: 'oxmux',
        kind: 'cli',
      },
      ...
      if (request.operation === 'command') {
        ...
        const output = await runCommand(
          ...
          command,
          {
            ...
            timeoutSeconds: readEnvNumber('OXMUX_OPENSANDBOX_COMMAND_TIMEOUT_SECONDS', DEFAULT_COMMAND_TIMEOUT_SECONDS),
          },
        )
        desktopState.lastOutput = output
        writePersistedState()
