...
const buildWorkerDoctor = async () => {
  ...
  const items: WorkerDoctorItem[] = [
    ...
    },
    {
      ...
      hint: config.executorId ? undefined : 'Run the installer command generated by oxmux, or enter the pairing code in the local worker console.',
    },
    {
      id: 'cloud-url',
      category: 'network',
    ...
    },
    {
      ...
      hint: meshConfig.enabled && (!easyTierCorePath || !easyTierCliPath || !easyTierConfigured)
        ? 'Install EasyTier and set OXMUX_EASYTIER_NETWORK_NAME, OXMUX_EASYTIER_NETWORK_SECRET, and OXMUX_EASYTIER_PEERS.'
        : undefined,
    },
  ]
