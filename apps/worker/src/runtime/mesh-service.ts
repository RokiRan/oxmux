...
const getMacOSLabel = (serviceName: string) => `com.oxmux.${serviceName}`
const getMacOSPlistPath = (serviceName: string) => `/Library/LaunchDaemons/${getMacOSLabel(serviceName)}.plist`
const getMacOSStdoutPath = (serviceName: string) => `/Library/Logs/oxmux/${serviceName}.stdout.log`
const getMacOSStderrPath = (serviceName: string) => `/Library/Logs/oxmux/${serviceName}.stderr.log`
...
${args}
    ...
    <dict>
        <key>OXMUX_WORKER_HOME</key>
        <string>${escapeXml(getWorkerHome())}</string>
        <key>OXMUX_WORKER_EXECUTABLE_PATH</key>
...
const installMacOSMeshService = async (serviceName: string) => {
  if (typeof process.getuid === 'function' && process.getuid() !== 0) {
    throw new Error(`Installing the macOS mesh service requires sudo. Run: sudo OXMUX_WORKER_HOME=${getWorkerHome()} ${process.argv[1]} mesh install-service`)
  }
  ...
  mkdirSync('/Library/Logs/oxmux', { recursive: true })
...
const uninstallMacOSMeshService = (serviceName: string) => {
  if (typeof process.getuid === 'function' && process.getuid() !== 0) {
    throw new Error(`Uninstalling the macOS mesh service requires sudo. Run: sudo OXMUX_WORKER_HOME=${getWorkerHome()} ${process.argv[1]} mesh uninstall-service`)
  }
