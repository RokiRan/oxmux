...
const resolveExistingCodeServerBin = async () => {
  const configured = readEnv('OXMUX_CODE_SERVER_BIN')
  if (configured) {
    if (await commandRuns(configured, ['--version'])) {
      return configured
    }
    throw new Error(`OXMUX_CODE_SERVER_BIN 不可执行或无法启动: ${configured}`)
  }
...
const installManagedCodeServer = async () => {
  ...
  if (process.platform === 'win32') {
    throw new Error('当前 worker 平台暂不支持自动安装 code-server，请设置 OXMUX_CODE_SERVER_BIN。')
  }
...
const pickPort = async () => {
  const configured = Number(readEnv('OXMUX_CODE_SERVER_PORT') || 0)
