...
export class WindowsService implements PlatformService {
  ...
  private serviceRoot() {
    return path.join(os.homedir(), 'AppData', 'Local', 'Oxmux', 'services', this.serviceName)
  }
  ...
  private startupShortcutPath() {
    ...
    return path.join(startupDir, `Oxmux-${this.serviceName}.vbs`)
  }
  ...
  private buildRegisterScheduledTaskCommand(config: WindowsServiceRuntimeConfig) {
    ...
    return [
      ...
      '-Trigger $trigger',
      '-Settings $settings',
      `-Description ${psQuote(`oxmux Worker (${this.serviceName})`)}`,
      '-Force',
    ].join(' ')
  }
