...
Oxmux 社区版默认开启**匿名使用上报**：你的实例会定期（每天一次 + 启动时）向 `https://oxmux.ai/api/community-usage/report` 发送一条极小的聚合报告，帮助我们了解社区版的真实使用情况，把维护精力花在真正被用到的功能上。
...
| `deploymentMode` | 仅当部署方显式设置 `OXMUX_DEPLOYMENT_MODE` 时携带 |
...
2. **自建端点覆盖**：把上报指向你自己的服务做审计或转发：
   ...
   ```bash
   OXMUX_USAGE_REPORTING_ENDPOINT=https://your-endpoint.example.com/report
   ```
...
上报的接收端点 `/api/community-usage/report` 默认关闭，只有 oxmux.ai 官方实例通过以下变量开启：

```bash
OXMUX_COMMUNITY_USAGE_COLLECTOR_ENABLED=1
```
