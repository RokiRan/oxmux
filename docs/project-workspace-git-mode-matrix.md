...
Oxmux 里有两条经常被混淆的维度，必须拆开看。

### 1.1 项目创建方式

项目创建方式描述的是：**项目目录最初是怎么进入 Oxmux 的**。
...
这两种主路径都应该落在 worker 的托管目录下，由 Oxmux 管理，而不是让用户在主流程里手选本地目录。
...
- 用户选择节点
- Oxmux 在该节点的托管目录下创建项目目录
- 初始状态通常为：
  - `versionControl = 'none'`
  - `gitUrl = ''`
- 这代表“目录已被 Oxmux 托管，但尚未初始化 Git”

### 2.2 Git Clone 项目

- 用户选择节点
- 用户提供 Git URL
- Oxmux 在该节点的托管目录下 clone 仓库
- 初始状态通常为：
