...
| 服务 | 地址 |
|------|------|
| 前端 | http://app.oxmux.localtest.me:15173 |
| 后端 | http://app.oxmux.localtest.me:18989 |
...
| `pnpm dev:preview` | https://oxmux.xyz/ |

## 常见问题

**端口占用**：先执行 `docker ps -a` 检查旧容器，再执行 `docker rm -f oxmux-postgres oxmux-rustfs oxmux-rustfs-init`

**需要重置基础设施**：执行 `pnpm dev:infra:down`，如需连数据一起重置，再执行 `docker volume rm oxmux_oxmux-postgres-data oxmux_oxmux-rustfs-data`
