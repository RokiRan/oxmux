...
> 记录 oxmux CLI 命令树、控制面 MCP 工具面及其单一事实源。
...
- **命名**：规范命令名 `oxmux`；`vbx` / `vibemux` 是旧别名（`getCliName()` 按启动 bin 名识别）；npm daemon 包 bin（`oxmux-worker*`）按旧入口处理（裸调用默认 `daemon`）
- **规范 CLI 判定**：`isCanonicalCliName()`（`oxmux`/`vbx`/`vibemux` 为规范 CLI，裸调用显示 help、顶层 worker 命令必须写 `oxmux worker <cmd>`）
- **认证**：`OXMUX_TOKEN`（走 `/mcp`）或已配对 worker 的 executor token（走 `/mcp/executor`）
...
- **端点**：`POST /mcp`（Bearer token，用户身份）与 `POST /mcp/executor`（executor token，可选 `x-oxmux-acting-user` 换身份）——`apps/server/src/routes/mcp-routes.ts`
- **组装**：`apps/server/src/integrations/mcp/oxmux-mcp-tools.ts` 聚合注册以下模块：
...
- **只读注解**：查询类工具注册时带 `OXMUX_READ_ONLY_MCP_TOOL_ANNOTATIONS`，新增查询工具须同步
- **回复路由**：inbox item 的 `replyTo` 字段决定回信地址（task_comment / channel / inbox_item），渠道差异只活在那里，投递方必须填、路由层不猜
- CLI 与 Agent 共用同一工具面：CLI 命令是 `OxmuxClient.callTool(name)` 的薄封装，**不为 CLI 发明服务端逻辑**
