...
// [OUTPUT]: 双前缀环境变量读取：`OXMUX_*` 优先，旧 `VIBEMUX_*` 兜底；以及启动期桥接函数。
//   桥接只同步非空 `OXMUX_*` 值——部署层常注入 `${OXMUX_X:-}` 空壳，空串不得覆盖 .env 已配的旧前缀真实值。
 ...
 * - 同名时 `OXMUX_` 优先于 `VIBEMUX_`。
 */

const toLegacyEnvName = (name: string) => {
  return name.startsWith('OXMUX_') ? `VIBEMUX_${name.slice('OXMUX_'.length)}` : name
}

/**
 * 双读读取：传新前缀名（如 `OXMUX_CLOUD_URL`），自动回退旧前缀。
...
/**
 * 启动期桥接：把已设置的 `OXMUX_*` 同步到 `VIBEMUX_*`，
 * 让仍直接读取旧前缀的运行时代码在兼容窗口内看到新值。
 * 需在 dotenv 加载之后、任何运行时环境读取之前调用。
 *
 * 注意：只同步**非空**值。部署层（如 docker-compose）常注入
 * `OXMUX_X=${OXMUX_X:-}` 空壳占位，若把空字符串写进 `VIBEMUX_*`，
 * 会覆盖 .env 里已配置的旧前缀真实值，导致配置静默失效。
 */
export const bridgeOxmuxEnvToLegacy = () => {
  for (const key of Object.keys(process.env)) {
    if (key.startsWith('OXMUX_') && process.env[key]?.trim()) {
